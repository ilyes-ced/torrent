...
fn bitfield(length: u32, index: [u8; 9]) -> [u8; 14] {
    ...
    [
        bytes[0], bytes[1], bytes[2], bytes[3], 5, index[0], index[1], index[2], index[3],
...
fn request(index: [u8; 4], begin: [u8; 4], length: [u8; 4]) -> [u8; 17] {
    ...
    [
        bytes[0], bytes[1], bytes[2], bytes[3], 6, index[0], index[1], index[2], index[3],
        begin[0], begin[1], begin[2], begin[3], length[0], length[1], length[2], length[3],
    ]
}
...
fn cancel(index: [u8; 4], begin: [u8; 4], length: [u8; 4]) -> [u8; 17] {
    let bytes = transform_u32_to_array_of_u8(13);
    [
        bytes[0], bytes[1], bytes[2], bytes[3], 8, index[0], index[1], index[2], index[3],
        begin[0], begin[1], begin[2], begin[3], length[0], length[1], length[2], length[3],
    ]
}
