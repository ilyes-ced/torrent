...
use crate::bencode::{decode::{DecoderElement}, self, encode};
...
impl Peers {
    ...
    pub fn get_peers(&mut self) -> Result<String, String> {
        ...
        buf[16..36].copy_from_slice(&self.info_hash); // info_hash
        buf[36..56].copy_from_slice(&self.peer_id); // peer_id
        buf[56..64].copy_from_slice(&[0, 0, 0, 0, 0, 0, 0, 0]); // downloaded
        buf[64..72].copy_from_slice(&self.size); // left
        buf[72..80].copy_from_slice(&[0, 0, 0, 0, 0, 0, 0, 0]); // uploaded
        buf[80..84].copy_from_slice(&[0, 0, 0, 0]); // event
        buf[84..88].copy_from_slice(&[0, 0, 0, 0]); // IP
        buf[88..92].copy_from_slice(&rng.gen::<[u8; 4]>()); // key
        buf[92..96].copy_from_slice(&[255, 255, 255, 255]); // num_want -1
        buf[96..98].copy_from_slice(&transform_u16_to_array_of_u8(PORT)); // port
        /*
        ...
        */
        ...
        while self.retry {
            ...
            self.retry_counter += 1;
            self.socket.send_to(&buf, self.socket_addr).unwrap();
            match self.socket.recv_from(&mut buffer){
                Ok(result) => {
                    self.retry = false;
                                        ...
                                        if let DecoderElement::Dict(file_details) = file {
                                            for details in file_details {
                                                if details.name == String::from("length"){
                                                    if let DecoderElement::Number(value) = details.value {
    ...
    fn info_hash(&mut self, decoded_file: DecoderElement) -> Result<(), String> {
        ...
        let info: Result<Vec<u8>, String> = {
            if let DecoderElement::Dict(pairs) = decoded_file {
                ...
                Err(String::from("info field not fount (might require nested searching feature)"))
            }else{
